...
def analizar_regex(
    ...
    for ref in dependencias_encontradas_raw:
        ...
        elif type_ref == 'external': dep_info = DependencyInfo(type='library', path=ruta_resuelta_o_original)
        elif type_ref in ['absoluta', 'relativa']:
            ruta_norm = normalizar_ruta(ruta_resuelta_o_original)
            key_to_check = ruta_norm
            logger.debug(f"  -> Ruta normalizada internal/broken: '{ruta_norm}'") # DEBUG
